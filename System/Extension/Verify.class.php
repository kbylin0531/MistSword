...
class Verify {
    ...
    public static function check($code, $id = '') {
        ...
        if(self::authcode(strtoupper($code)) == $secode['verify_code']) {
            self::$config['reset'] && SessionUtil::set($key, null);
            return true;
        }
        return false;
    }
    ...
    public static function entry($id = '') {
        // 图片宽(px)
        if(self::$config['imageW']){
            self::$config['imageW'] = self::$config['length']*self::$config['fontSize']*1.5 +
                self::$config['length']*self::$config['fontSize']/2;
        }
        // 图片高(px)
        if(self::$config['imageH']){
            self::$config['imageH'] = self::$config['fontSize'] * 2.5;
        }
        // 建立一幅 self::imageW x self::imageH 的图像
        self::$_image = imagecreate(self::$config['imageW'], self::$config['imageH']);
        // 设置背景
        imagecolorallocate(self::$_image, self::$config['bg'][0], self::$config['bg'][1], self::$config['bg'][2]);
        // 验证码字体随机颜色
        self::$_color = imagecolorallocate(self::$_image, mt_rand(1,150), mt_rand(1,150), mt_rand(1,150));
        // 验证码使用随机字体
        $ttfPath = dirname(__FILE__) . '/Verify/' . (self::$config['useZh'] ? 'zhttfs' : 'ttfs') . '/';
        if(empty(self::$config['fontttf'])){
            $dir = dir($ttfPath);
            $ttfs = array();
            ...
            $dir->close();
            self::$config['fontttf'] = $ttfs[array_rand($ttfs)];
        }
        self::$config['fontttf'] = $ttfPath . self::$config['fontttf'];
        if(self::$config['useImgBg']) {
            self::_background();
        }
        if (self::$config['useNoise']) {
            // 绘杂点
            self::_writeNoise();
        }
        if (self::$config['useCurve']) {
            // 绘干扰线
            self::_writeCurve();
        }
        // 绘验证码
        ...
        if(self::$config['useZh']){ // 中文验证码
            for ($i = 0; $i<self::$config['length']; $i++) {
                $code[$i] = iconv_substr(self::$config['zhSet'],floor(mt_rand(0,mb_strlen(self::$config['zhSet'],'utf-8')-1)),1,'utf-8');
                imagettftext(self::$_image, self::$config['fontSize'], mt_rand(-40, 40), self::$config['fontSize']*($i+1)*1.5, self::$config['fontSize'] + mt_rand(10, 20), self::$_color, self::$config['fontttf'], $code[$i]);
            }
        }else{
            for ($i = 0; $i<self::$config['length']; $i++) {
                $code[$i] = self::$config['codeSet'][mt_rand(0, strlen(self::$config['codeSet'])-1)];
                $codeNX  += mt_rand(self::$config['fontSize']*1.2, self::$config['fontSize']*1.6);
                imagettftext(self::$_image, self::$config['fontSize'], mt_rand(-40, 40), $codeNX, self::$config['fontSize']*1.6, self::$_color, self::$config['fontttf'], $code[$i]);
            }
        }
        // 保存验证码
        $key        =   self::authcode(self::$config['seKey']);
        $code       =   self::authcode(strtoupper(implode('', $code)));
        $secode     =   array();
    ...
    private static function _writeCurve() {
        $py = 0;
        // 曲线前部分
        $A = mt_rand(1, self::$config['imageH']/2);                  // 振幅
        $b = mt_rand(-self::$config['imageH']/4, self::$config['imageH']/4);   // Y轴方向偏移量
        $f = mt_rand(-self::$config['imageH']/4, self::$config['imageH']/4);   // X轴方向偏移量
        $T = mt_rand(self::$config['imageH'], self::$config['imageH']*2);  // 周期
        $w = (2* M_PI)/$T;
        $px1 = 0;  // 曲线横坐标起始位置
        $px2 = mt_rand(self::$config['imageW']/2, self::$config['imageW'] * 0.8);  // 曲线横坐标结束位置
        for ($px=$px1; $px<=$px2; $px = $px + 1) {
            if ($w!=0) {
                $py = $A * sin($w*$px + $f)+ $b + self::$config['imageH']/2;  // y = Asin(ωx+φ) + b
                $i = (int) (self::$config['fontSize']/5);
                while ($i > 0) {
        ...
        $T = mt_rand(self::$config['imageH'], self::$config['imageH']*2);  // 周期
        $w = (2* M_PI)/$T;
        $b = $py - $A * sin($w*$px + $f) - self::$config['imageH']/2;
        $px1 = $px2;
        $px2 = self::$config['imageW'];
        for ($px=$px1; $px<=$px2; $px=$px+ 1) {
            if ($w!=0) {
                $py = $A * sin($w*$px + $f)+ $b + self::$config['imageH']/2;  // y = Asin(ωx+φ) + b
                $i = (int) (self::$config['fontSize']/5);
    ...
    private static function _writeNoise() {
        ...
        for($i = 0; $i < 10; $i++){
            ...
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring(self::$_image, 5, mt_rand(-10, self::$config['imageW']),  mt_rand(-10, self::$config['imageH']), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
    /**
    ...
    private static function _background() {
        ...
        @imagecopyresampled(self::$_image, $bgImage, 0, 0, 0, 0, self::$config['imageW'], self::$config['imageH'], $width, $height);
    ...
    private static function authcode($str){
        $key = substr(md5(self::$config['seKey']), 5, 8);
